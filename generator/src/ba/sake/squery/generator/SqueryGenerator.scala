...
class SqueryGenerator(connection: Connection, config: SqueryGeneratorConfig = SqueryGeneratorConfig.Default) {
  private val logger = Logger(getClass.getName)

  private val Preamble = "/* Autogenerated and maintained by Squery generator */"
  private val merger = SourceMerger(mergeDefBodies = false)
  private val generator = RegenescaGenerator(merger)

  def generateString(schemaNames: Seq[String]): String = {
    val extractor = DbDefExtractor(connection)
...
object NameMapper {
  ...
  def apply(name: String): NameMapper = name.toLowerCase() match {
    case "noop"       => Noop
    case "camelcase"  => CamelCase
    case other        => throw new IllegalArgumentException(s"Unknown NameMapper: $other")
  }
}
